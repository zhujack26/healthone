...
const ReportManageTable = ({showAnswerModal, tableColumnName}) => {
  ...
  const reportInfoData = new ReportInfoData(1, "hong123@gmail.com", "로그인이 안돼요", "2023. 04. 12.", "로그인/회원가입")
  const size = 20;
  ...
  const [reportTableItem, setReportTableItem] = useState(
    [Array(size)
      .fill()
      .map((e, i) =>
        <ReportTableItem ReportInfoData={reportInfoData} showAnswerModal={() => { showAnswerModal() }} key={i} />
    )
    ]
  );
